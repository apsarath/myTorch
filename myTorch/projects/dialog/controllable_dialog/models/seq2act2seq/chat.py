...
def run_epoch(epoch_id, mode, experiment, model, config, data_reader, tr, logger, device):
    ...
    for mini_batch in itr:
        ...
        samples = []
        for e_id in range(mini_batch["sources"].shape[0]):
            
            
            def beam_search(k, input_):

        output_logits, curr_act_logits, next_act_logits = model(
                        mini_batch["sources"][e_id].to(device),
                        mini_batch["sources_len"][e_id].to(device),
                        mini_batch["targets_input"][e_id].to(device),
