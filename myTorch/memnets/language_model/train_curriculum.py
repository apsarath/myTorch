...
from myTorch.memnets.utils.curriculum import curriculum_generator

import torch.nn.functional as F

parser = argparse.ArgumentParser(description="Algorithm Learning Task")
parser.add_argument("--config", type=str, default="/Users/shagun/projects/myTorch/myTorch/memnets/language_model/ptb.yaml", help="config file path.")
...
def run_experiment(args):
    ...
    for curriculum_idx, curriculum_config in enumerate(curriculum_generator(config)):
        logging.info("\nStarting curriculum index: {}".format(curriculum_idx))

        logging.info(curriculum_config.get())

        experiment, model, batched_data, tr, logger, device = create_experiment(curriculum_config)
        ...
        for i in range(curriculum_config.num_epochs):
            logging.info("\n#####################\n Epoch id: {}\n".format(i+1))
            for mode in ["train", "valid", "test"]:
                tr.mini_batch_id[mode] = 0
                run_epoch(i, mode, experiment, model, curriculum_config, batched_data, tr, logger, device)
