...
class DecoderRNN(nn.Module):
    ...
    def reset_hidden(self, batch_size):
        self._hidden = {}
        self._hidden["h"] = my_variable(torch.zeros(batch_size, self._hidden_size), use_gpu=self._use_gpu)
        self._hidden["c"] = my_variable(torch.zeros(batch_size, self._hidden_size), use_gpu=self._use_gpu)
    ...
    def forward(self, inputs, encoder_outputs):
        ...
        outputs = []
        for input in inputs:
            output, hidden_next = self._rnn(torch.cat((input, encoder_outputs[-1]),1), self._hidden)
