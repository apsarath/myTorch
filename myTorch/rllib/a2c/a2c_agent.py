...
class A2CAgent(object):
    ...
    def sample_action(self, obs, dones=[], legal_moves=None, is_training=True, update_agent_state=True):
        obs = torch.from_numpy(obs).type(torch.FloatTensor).to(self._a2cnet.device)
        if len(dones) > 0:
        ...
        if legal_moves is None:
                legal_moves = torch.ones_like(pvals).to(self._a2cnet.device)
        ...
        return actions.cpu().numpy().squeeze(1), log_taken_pvals.squeeze(1), vvals.squeeze(1), entropies, pvals
    ...
    def train_step(self, minibatch):
        ...
        return pg_loss.item(), val_loss.item(), entropy_loss.item()
